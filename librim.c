...
rim_t* blank_rim(unsigned int width, unsigned int height, rim_colorfmt_t colorfmt, bool do_palette)
{
    rim_t* rim = malloc_rim();
    if (rim == NULL){return NULL;}
    rim->width = width;
    rim->height = height;
    rim->colorfmt = colorfmt;
    if (do_palette)
    {
        rim->palette = calloc(256*colorfmt, 1);
        rim->pixels = calloc(width*height, 1);
    }
    else
    {
        rim->palette = NULL;
        rim->pixels = calloc(width*height*colorfmt, 1);
    }
    return rim;
}
